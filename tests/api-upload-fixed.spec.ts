...
test.describe('ファイルアップロードAPI', () => {
  ...
  test('正常系：画像ファイルのアップロード', async ({ request }) => {
    ...
    formData.append('image', new Blob([imageData.buffer], { type: 'image/png' }), 'test.png');
  ...
  test('正常系：JPEGファイルのアップロード', async ({ request }) => {
    ...
    const formData = new FormData();
    formData.append('image', new Blob([imageData.buffer], { type: 'image/jpeg' }), 'test.jpg');
  ...
  test('異常系：画像以外のファイルをアップロード', async ({ request }) => {
    ...
    const formData = new FormData();
    formData.append('image', new Blob([textData.buffer], { type: 'text/plain' }), 'test.txt');
  ...
  test('正常系：複数ファイルの連続アップロード', async ({ request }) => {
    ...
    for (const file of files) {
      const formData = new FormData();
      formData.append('image', new Blob([file.data.buffer], { type: 'image/png' }), file.name);
  ...
  test('正常系：アップロードされたファイルにアクセス可能', async ({ request }) => {
    ...
    const formData = new FormData();
    formData.append('image', new Blob([imageData.buffer], { type: 'image/png' }), 'test-access.png');
