...
### GC（ガベージコレクション）機能
- 現行形式（文字列の`lines`）と旧形式（`text`付きオブジェクト）のCosenseバックアップJSONを解析
- 不要ファイルの自動削除
- 削除結果の詳細表示
- Cosenseバックアップとして検証できないJSONでは削除を実行しない
...
2. 画像ファイルをドラッグ&ドロップ、貼り付け、またはファイル選択
3. 選択直後に自動アップロードされ、生成されたリンクがクリップボードにコピーされる
4. 履歴から再度コピーする場合、コピーボタンまたはリンク行をクリックする
